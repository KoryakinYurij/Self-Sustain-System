# Codex Skill Creator Prompt

Оригинальный системный промпт (instruction), используемый OpenAI Codex для генерации и структурирования новых скиллов.
...
Validate the skill (run quick_validate.py)
...
Added scripts must be tested by actually running them to ensure there are no bugs and that the output matches ক্ষমতায় what is expected. If there are many similar scripts, only a representative sample needs to be tested to ensure confidence that they all work while balancing time to completion.
